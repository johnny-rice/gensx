...
suite("FileSystemBlobStorage", () => {
  ...
  test("should list blobs with prefix", async () => {
    ...
    expect(result.blobs.map((blob) => blob.key)).toContain("prefix1/a");
    expect(result.blobs.map((blob) => blob.key)).toContain("prefix1/b");
    expect(
      result.blobs.every((blob) => typeof blob.lastModified === "string"),
    ).toBe(true);
    expect(result.blobs.every((blob) => typeof blob.size === "number")).toBe(
      true,
    );

    const prefix2Result = await storage.listBlobs({ prefix: "prefix2" });
    expect(prefix2Result.blobs).toHaveLength(1);
    expect(prefix2Result.blobs.map((blob) => blob.key)).toContain("prefix2/c");

    const allBlobs = await storage.listBlobs();
    expect(allBlobs.blobs).toHaveLength(3);
  });
  ...
  test("should respect default prefix", async () => {
    ...
    expect(blobs.blobs.map((blob) => blob.key)).toContain("test1");
    expect(blobs.blobs.map((blob) => blob.key)).toContain("test2");
  ...
  test("should handle default prefix in listBlobs", async () => {
    ...
    expect(result.blobs).toHaveLength(2);
    expect(result.blobs.map((blob) => blob.key)).toContain("key1");
    expect(result.blobs.map((blob) => blob.key)).toContain("key2");
  });
  ...
  test("should combine default prefix with provided prefix in listBlobs", async () => {
    ...
    expect(result.blobs).toHaveLength(2);
    expect(result.blobs.map((blob) => blob.key)).toContain("sub/key1");
    expect(result.blobs.map((blob) => blob.key)).toContain("sub/key2");
    expect(result.blobs.map((blob) => blob.key)).not.toContain("other/key3");
  });
  ...
  test("should handle pagination with limit", async () => {
    ...
    expect(firstPage.blobs.map((blob) => blob.key)).toEqual(["test1", "test2"]);
    expect(firstPage.nextCursor).not.toBeUndefined();
    ...
    expect(secondPage.blobs).toHaveLength(2);
    expect(secondPage.blobs.map((blob) => blob.key)).toEqual([
      "test3",
      "test4",
    ]);
    expect(secondPage.nextCursor).not.toBeUndefined();
    ...
    expect(lastPage.blobs).toHaveLength(1);
    expect(lastPage.blobs.map((blob) => blob.key)).toEqual(["test5"]);
  ...
  test("should handle pagination with prefix", async () => {
    ...
    expect(firstPage.blobs).toHaveLength(2);
    expect(firstPage.blobs.map((blob) => blob.key)).toEqual([
      "prefix1/a",
      "prefix1/b",
    ]);
    expect(firstPage.nextCursor).not.toBeUndefined();
    ...
    expect(secondPage.blobs.map((blob) => blob.key)).toEqual(["prefix1/c"]);
  ...
  test("should handle default limit in pagination", async () => {
    ...
    const firstPage = await storage.listBlobs();
    expect(firstPage.blobs.length).toBe(100); // Default limit
    expect(firstPage.nextCursor).not.toBeUndefined();
    ...
    expect(secondPage.blobs.length).toBe(50);
    expect(secondPage.nextCursor).toBeUndefined();
  });
});
